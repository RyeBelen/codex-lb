...
def responses_source_route_excluded(
    ...
    *,
    ...
    A terminal compaction trigger is forwarded through the normal Responses
    stream on the turn's owner account (Codex path only — callers set
    ``exclude_compaction=False`` on ``/v1/responses``). An
    ``input_file``/``input_image`` file reference is
