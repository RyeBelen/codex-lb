...
from app.modules.proxy._service.streaming.helpers import _MODEL_CAPACITY_LIMIT_CODES
...
_PORTABLE_ENCRYPTED_INPUT_FIELDS = {
    "compaction": frozenset({"encrypted_content", "id", "status", "type"}),
    "reasoning": frozenset({"encrypted_content", "id", "status", "summary", "type"}),
}


def _portable_encrypted_input_item(item: Any) -> bool:
    if not isinstance(item, dict):
        return False
    item_type = item.get("type")
    if not isinstance(item_type, str):
        return False
    allowed_fields = _PORTABLE_ENCRYPTED_INPUT_FIELDS.get(item_type)
    if allowed_fields is None or any(field not in allowed_fields for field in item):
        return False
    ...
    if item.get("id") is not None and not isinstance(item["id"], str):
        return False
    if item.get("status") is not None and not isinstance(item["status"], str):
        return False
...
def _payload_allows_previsible_compaction_spillover(payload: ResponsesRequest) -> bool:
    replay_payload = dict(payload.to_replay_safety_payload())
    context_management = replay_payload.pop("context_management", None)
    if context_management is not None and not (
        isinstance(context_management, list)
        and all(
            isinstance(control, dict)
            and set(control) == {"compact_threshold", "type"}
            and control.get("type") == "compaction"
            and isinstance(control.get("compact_threshold"), int)
            and not isinstance(control["compact_threshold"], bool)
            and cast(int, control["compact_threshold"]) > 0
            for control in context_management
        )
    ):
        return False
    input_items = replay_payload.get("input")
    if not isinstance(input_items, list):
        return False
    portable_items = [item for item in input_items if _portable_encrypted_input_item(item)]
    if not portable_items:
        return False
    replay_payload["input"] = [item for item in input_items if item not in portable_items]
            ...
            for attempt in range(max_attempts):
                ...
                try:
                    ...
                    if (
                        ...
                        and not require_preferred_account
                        and file_preferred_account_id is None
                        and exc.code in _MODEL_CAPACITY_LIMIT_CODES
                        and _payload_allows_previsible_compaction_spillover(payload)
                    ):
                        payload_replay_required_account_id = None
                        affinity = replace(affinity, reallocate_sticky=True)
                        logger.info(
                            "portable_compaction_spillover request_id=%s account_id=%s code=%s",
                            request_id,
                            account.id,
                            exc.code,
                        )
