...
async def resolve_account_scope(api_key: ApiKeyData | None, *, model: str | None = None) -> set[str] | None:
    scoped = (
    ...
    async with get_background_session() as session:
        ...
        model_scope = await ModelRoutingRepository(session).scope(model)
        if model_scope is not None:
            return model_scope if scoped is None else scoped & model_scope
        return scoped


async def require_account_access(account_id: str, api_key: ApiKeyData | None, *, model: str | None = None) -> None:
    outside_key_scope = (
        api_key is not None
