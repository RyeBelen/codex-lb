...
- [x] 4.2 Add the `Reset (N)` button to `frontend/src/features/accounts/components/account-actions.tsx` immediately after the Export button, matching its `size="sm" variant="outline" className="h-8 gap-1.5 text-xs"` style, with a `RotateCcw` icon, a single-unit countdown label (using 3.3) placed at the button's right-upper radius, and destructive/red label color when `expiringSoon`. Keep it visible but disabled when `availableResetCredits` is `0`. Wire `onClick` to open the confirmation dialog only when enabled
- [x] 4.3 Add a reset action to `frontend/src/features/dashboard/components/account-list.tsx` (table view) inside the existing Details action cell, matching the `h-7 w-7` icon-button style with the countdown and count exposed in the `title` tooltip. Keep it visible but disabled when `availableResetCredits` is `0`
- [x] 4.4 Add a `Reset (N)` button to `frontend/src/features/dashboard/components/account-card.tsx` (grid view) next to the Details button, matching the `h-7 gap-1.5` text style with the single-unit countdown label. Keep it visible but disabled when `availableResetCredits` is `0`
...
- [x] 5.8 Frontend — Reset button visibility: rendered in all three surfaces (account-actions, dashboard table, dashboard grid) and disabled when `availableResetCredits` is `0`
